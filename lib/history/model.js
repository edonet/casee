...
export default function createHistory(options, history) {
    let { publish, subscribe } = createObserver(),
        model = {},
        unlisten;


    /* 创建历史对象 */
    model.history = history || createHashHistory(options);
    ...
    /* 监听历史记录变化 */
    unlisten = model.history.listen(updateHistory(model, publish));
 ...
 *****************************************
 */
function updateHistory(model, publish) {
