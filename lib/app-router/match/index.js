...
export default function matchState(router) {
    return function match(url, callback) {
        let { pathname, method, context = '/' } = router,
            path = resolvePath(context)(url || './:__404__'),
            matched = context.endsWith('/:__404__') ? null : matchPath(pathname, { path });
...
export function updateAppMatcher() {
    ...
    // 设置更新定时器
    model.timeStamp = setTimeout(invokeRouterCallback, 100);
}
 ...
 *****************************************
 */
export function routeWillUpdate() {
    model.count ++;
 ...
 *****************************************
 */
export function routeDidUpdate() {
    model.count --;
