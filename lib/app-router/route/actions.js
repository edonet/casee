...
export default function createHistory(context) {
    ...
    if (!(context in model)) {
        ...
        history.go = (path, { method = 'PUSH' } = {}) => (
            updateRouterState({ pathname: resolve(path), action: 'PUSH', method })
        );

        // 替换路径
        history.replace = (path, { method = 'REPLACE' } = {}) => (
            updateRouterState({ pathname: resolve(path), action: 'REPLACE', method })
        );
        ...
        history.goBack = (step = -1, { method = 'POP' } = {}) => {
            ...
            if (typeof step === 'string') {
                return updateRouterState({ pathname: resolve(step), action: 'POP', method });
            }

            // 返回步数
            if (typeof step === 'number') {
                return updateRouterState({ step, action: 'POP', method });
            }
        };
