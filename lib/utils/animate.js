 ...
 *****************************************
 */
export const updater = cb => handler => {
    cb || rAF(() => cb = cb && cb() && null);
    cb = handler;
};
 ...
 *****************************************
 */
export const momentum = (speed, obstruct = .005) => {
    let duration = Math.abs(speed) / obstruct;

    return {
        duration, distance: duration * speed / 2
    };
};
