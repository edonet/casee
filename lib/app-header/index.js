...
export default class AppHeader extends Component {
    ...
    render() {
        ...
        return show ? (
            <header className="app-header abs box row tc">
                <div className="app-nav-left flex rel row">
                    { this.renderNavBack(navBack) }
                    <AppTransition className="row" component={ navLeft || 'left' } />
                </div>
                <h1 className="app-title flex rel">
    ...
    renderNavBack(navBack) {
        let classList = ['app-nav-back row pv5 pl15 pr5'];

        // 添加样式
        navBack || classList.push('disabled');

        // 返回元素
        return (
            <AppToucher className={ classList.join(' ') } onTap={ this.handleNavBack }>
                <AppIcon className="f12" name="arrow-left" />
                <span>返回</span>
            </AppToucher>
        );
    }
